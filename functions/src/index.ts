...
const pickTitle = (x: any) =>
  x?.title || x?.name || x?.volumeInfo?.title || x?.snippet?.title;

const pickAuthorsArray = (x: any): string[] => {
  if (Array.isArray(x?.authors)) return x.authors;
  if (Array.isArray(x?.volumeInfo?.authors)) return x.volumeInfo.authors;
  return [];
};
...
function extractExplicitTopic(utter: string): { kind: "book"|"video"|null; term: string|null } {
  const u = (utter || "").toLowerCase().trim();
  if (!u) return { kind: null, term: null };
  ...
  if (m2) {
    ...
    if (term && !GENRE_ALIASES[term]) return { kind, term };
  }
...
function containsAny(hay: string | null | undefined, needles: string[]) {
  if (!hay) return false;
  const lc = hay.toLowerCase();
  return needles.some(n => lc.includes(n));
}
function bookMatchesTopic(raw: any, topic: string) {
...
function videoMatchesTopic(raw: any, topic: string) {
  const vars = topicVariants(topic);
  const title = pickTitle(raw);
...
/* ---- display simplifier ---- */
function simplifyItem(kind: "book" | "video", raw: any) {
  return {
    id: kind === "book"
...
/* ---- de-dupe helper ---- */
function uniqNew(items: any[], seen: Set<string>, kind: "book"|"video") {
  const out: any[] = [];
...
async function fetchBooksBySearch(
  term: string,
  opts: { page: number; pageSize: number; lang?: string }
) {
  const q = term.trim();
  if (API_BASE) {
    const u = new URL(`${API_BASE.replace(/\/+$/, "")}/api/books`);
    ...
    try {
      ...
      if (isBooks) {
        ...
        if (freeBookQuery) {
          ...
          const filtered = items.filter((it: any) => bookMatchesTopic(it, freeBookQuery));
        ...
        for (const it of topItems) { const id = idForBook(it); if (id) seenBooks.add(id); }

        const remembered = {
          kind: "book" as const,
          category: String(freeBookQuery || bookCanon),
          seedTitle: topItems[0] ? (pickTitle(topItems[0]) || "") : "",
          items: topItems.map((it: any) => simplifyItem("book", it))
        };
      ...
      if (isVideos) {
        ...
        const vFirst = await fetchVideosByTopic(
          (videoCanon || "kids") as Canon,
          { startIndex: 0, lang, pageToken: null, freeQuery: freeVideoQuery || null }
        );
        ...
        for (const it of topItems) { const id = idForVideo(it); if (id) seenV.add(id); }

        const remembered = {
          kind: "video" as const,
          topic: String(freeVideoQuery || videoCanon),
          seedTitle: topItems[0] ? (pickTitle(topItems[0]) || "") : "",
          items: topItems.map((it: any) => simplifyItem("video", it))
        };
      ...
      if (tagKey === "more_like_this") {
        const p = (req.body?.sessionInfo?.parameters as any) || {};
        ...
        if (kind === "book") {
          ...
          for (let tries = 0; tries < 10 && fresh.length < 5; tries += 1) {
            const page = await fetchBooksByCategory(String(category) as Canon, {
            ...
            if (uniques.length > 0) {
              ...
              if (page.source === "app") {
                const { term, juvenile } = bookQueryFor(String(category) as Canon);
                const g = new URL("https://www.googleapis.com/books/v1/volumes");
                g.searchParams.set("q", `${term}${juvenile ? " subject:juvenile" : ""}`);
          ...
          const list = topItems.map((it: any, i: number) => `${i + 1}. ${pickTitle(it) ?? "Untitled"}`).join("\n");
          ...
          for (const it of topItems) { const id = idForBook(it); if (id) seen.add(id); }
        ...
        for (let tries = 0; tries < 10 && freshV.length < 5; tries += 1) {
          const page = await fetchVideosByTopic(String(topic) as Canon, { startIndex, lang: p.language, pageToken });
          ...
          if (uniques.length > 0) {
            ...
            if (page.source === "app") {
              const q = videoQueryFor(String(topic) as Canon);
              const y = new URL("https://www.googleapis.com/youtube/v3/search");
              y.searchParams.set("part", "snippet");
              y.searchParams.set("type", "video");
              y.searchParams.set("videoEmbeddable", "true");
        ...
        const listV = topItemsV.map((it: any, i: number) => `${i + 1}. ${pickTitle(it) ?? "Untitled"}`).join("\n");
        ...
        const rememberedV = {
          ...
          items: topItemsV.map((it: any) => simplifyItem("video", it))
        };

        for (const it of topItemsV) { const id = idForVideo(it); if (id) seenV.add(id); }
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      for (const it of items) {
        ...
        if (!vec || !Array.isArray(vec) || vec.length === 0) throw new Error(`item ${it.id} missing vector or text`);
  ...
  async (req, res): Promise<void> => {
    try {
      const { text, vector, filters, k } = (req.body ?? {}) as {
        ...
        filters?: { kind?: string; title_ilike?: string };
        k?: number;
      };

      let qv = vector as number[] | undefined;
...
export const embedItems = onRequest(async (req, res) => {
  try {
    ...
    if (!Array.isArray(texts) || !texts.length) {
      res.status(400).json({ error: "Provide texts: string[]" }); return;
    }
