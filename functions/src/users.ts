...
export async function rebuildUserProfile(userId: string) {
  ...
  let v: number[] = new Array(1536).fill(0);
  if (ids.length) {
    const { rows } = await pool.query<{ embedding: number[] }>(
      `SELECT embedding FROM items WHERE id = ANY($1::text[])`, [ids]
    );
    rows.forEach((r: { embedding: number[] }) => add(v, r.embedding, 1));      // typed
    rows.slice(0, favIds.length).forEach((r: { embedding: number[] }) => add(v, r.embedding, 1));
  }
  interestVecs.forEach((iv: number[]) => add(v, iv, 0.5));
