...
exports.recommendForUser = (0, https_1.onRequest)({ timeoutSeconds: 120 }, async (req, res) => {
    try {
        ...
        if (prof.rows.length > 0) {
            ...
            const params = [v, restrictions, seenIds];
            if (type)
                params.push(type);
            if (childAge !== undefined)
                params.push(childAge);
            const q = await pool.query(`SELECT id, title, description, type
           FROM items
          WHERE ${where}
        ...
        const paramsG = [restrictions, seenIds];
        const whereG = [
